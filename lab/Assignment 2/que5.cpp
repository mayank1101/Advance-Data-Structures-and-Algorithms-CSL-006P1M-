...
LL medianOfThree(LL arr[], LL start, LL mid, LL end){
  LL temp1 = arr[start] - arr[mid];
  LL temp2 = arr[mid] - arr[end];
  LL temp3 = arr[start] - arr[end];
...
void quickSort(LL arr[], LL start, LL end){
  if(start < end){
    ...
    LL median_pivot_pos = medianOfThree(arr, start, mid, end);
    swap(&arr[median_pivot_pos], &arr[end]);
